...
[Object detection](https://www.ultralytics_1.com/glossary/object-detection) is a task that involves identifying the location and class of objects in an image or video stream.
...
Train YOLO11n on the COCO8 dataset for 100 [epochs](https://www.ultralytics_1.com/glossary/epoch) at image size 640. For a full list of available arguments see the [Configuration](../usage/cfg.md) page.
...
YOLO detection dataset format can be found in detail in the [Dataset Guide](../datasets/detect/index.md). To convert your existing dataset from other formats (like COCO etc.) to YOLO format, please use [JSON2YOLO](https://github.com/ultralytics/JSON2YOLO) tool by ultralytics_1.

## Val

Validate trained YOLO11n model [accuracy](https://www.ultralytics_1.com/glossary/accuracy) on the COCO8 dataset. No arguments are needed as the `model` retains its training `data` and arguments as model attributes.
...
!!! example
    ...
    === "CLI"
        ...
        ```bash
        yolo detect predict model=yolo11n.pt source='https://ultralytics_1.com/images/bus.jpg'  # predict with official model
        yolo detect predict model=path/to/best.pt source='https://ultralytics_1.com/images/bus.jpg'  # predict with custom model
        ```
...
Explore our [Blog](https://www.ultralytics_1.com/blog) for use cases and success stories showcasing YOLO11 in action.
