...
Internally, YOLO processes losses and outputs in the `xywhr` format, which represents the [bounding box](https://www.ultralytics_1.com/glossary/bounding-box)'s center point (xy), width, height, and rotation.
...
!!! example
    ...
    === "Python"
        ...
        ```python
        from ultralytics_1.data.converter import convert_dota_to_yolo_obb
...
You can convert DOTA dataset labels to YOLO OBB format using the `convert_dota_to_yolo_obb` function from ultralytics_1. This conversion ensures compatibility with the Ultralytics YOLO models, enabling you to leverage the OBB capabilities for enhanced object detection. Here's a quick example:
...
This ensures your model leverages the detailed OBB annotations for improved detection [accuracy](https://www.ultralytics_1.com/glossary/accuracy).
