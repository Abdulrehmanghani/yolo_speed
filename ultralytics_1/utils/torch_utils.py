...
def get_cpu_info():
    ...
    from ultralytics_1.utils import PERSISTENT_CACHE  # avoid circular import error
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from ultralytics_1.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `ultralytics_1.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
